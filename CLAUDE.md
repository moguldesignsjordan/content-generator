...
**Verify UI changes in the real app, not just at the type level.** A green typecheck/build says the code compiles, not that the screen is right. For **backend/logic changes** (`lib/pipeline/`, `lib/db/`, prompt assembly, cost, CTA/`{$unsubscribe}` rules, Portable Text conversion), `npm run typecheck` + `npm run build` + `npm test` (Vitest) is the complete gate — no browser needed. Reach for the browser only when the thing that could break is visible **at runtime, in a browser**: rendering with real data, interaction/state (click Generate → draft appears, click-to-edit → save persists), an end-to-end round-trip (form → API → DB → re-render), or when "looks right" is the bar (layout, mobile, light/dark).

**Playwright MCP is disabled by default to keep context lean** (its ~21 browser-tool schemas were ~a third of context). It lives in `.mcp.json` but is switched off via `disabledMcpjsonServers` in `.claude/settings.local.json`. When a change genuinely needs browser verification, turn it on for that session — flip `disabledMcpjsonServers` → `enabledMcpjsonServers` in that file (or `/mcp`), start a fresh session, then drive the flow against `npm run dev` at `http://localhost:3000` (navigate, exercise, snapshot/screenshot, check light + dark). Flip it back off when done. If the MCP is off and only a screenshot is needed, a throwaway `npx playwright` script via Bash also works.
