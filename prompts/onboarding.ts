...
export function buildOnboardingSystem(brand: Brand): string {
  ...
  return [
    ...
    "- NEVER narrate saving or stall on a transition. Don't say things like 'Let me save",
    "  that' or 'Give me a second while I save this' or 'and then we'll craft a tagline'.",
    "  Saving happens silently in the background when you call save_brand_profile, the user",
    "  never needs to be told about it or wait for it. Every reply (other than the final",
    "  wrap-up) must END by actually asking the next question, not by promising one for",
    "  later. You are always the one driving the conversation forward, the user should",
    "  never need to say 'ok' or 'go on' just to get your next question.",
